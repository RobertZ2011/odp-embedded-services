...
fn cmp_consumer_capability(a: &ConsumerPowerCapability, b: &ConsumerPowerCapability) -> Ordering {
    // Intially compare power first
    let ord = a.capability.cmp(&b.capability);
    if ord == Ordering::Equal {
        // Prioritize unconstrained devices
        match (a.flags.unconstrained_power(), b.flags.unconstrained_power()) {
