...
pub type DeviceType = Mutex<GlobalRawMutex, Mock<'static, DynamicSender<'static, RequestData>>>;
...
async fn power_policy_task<const N: usize>(
    ...
    mut event_receivers: EventReceivers<'static, N, DeviceType, DynamicReceiver<'static, RequestData>>,
) {
...
pub async fn run_test<F: Future<Output = ()>>(
    ...
    let psu_registration = POWER_POLICY_PSU_REGISTRATION.init([
        psu::RegistrationEntry::new(DeviceId(0), device0),
        psu::RegistrationEntry::new(DeviceId(1), device1),
    ]);
    ...
    with_timeout(
        ...
        join(
            power_policy_task(
                completion_signal,
                power_policy,
                EventReceivers::new(psu_registration, [device0_receiver, device1_receiver]),
            ),
            async {
