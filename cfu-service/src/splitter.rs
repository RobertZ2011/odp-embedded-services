...
impl<'a, C: Customization> Splitter<'a, C> {
    ...
    ///
    /// This function will execute the closure concurrently in groups up to 4 items at a time.
    async fn map_slice_join<'i, 'o, I, O, F: Future<Output = bool>>(
        input: &'i [I],
        output: &'o mut [O],
        f: impl Fn(&'i I, &'o mut O) -> F,
    ) -> bool {
        ...
        loop {
            match (iter.next(), iter.next(), iter.next(), iter.next()) {
                ...
                (Some((i0, o0)), None, None, None) => return f(i0, o0).await,
                (Some((i0, o0)), Some((i1, o1)), None, None) => {
                    return join_array([f(i0, o0), f(i1, o1)]).await.into_iter().all(|x| x);
                }
                (Some((i0, o0)), Some((i1, o1)), Some((i2, o2)), None) => {
                    return join_array([f(i0, o0), f(i1, o1), f(i2, o2)])
                        .await
                        .into_iter()
                        .all(|x| x);
                }
                (Some((i0, o0)), Some((i1, o1)), Some((i2, o2)), Some((i3, o3))) => {
                    if !join_array([f(i0, o0), f(i1, o1), f(i2, o2), f(i3, o3)])
                        .await
                        .into_iter()
                        .all(|x| x)
                    {
                        // Only exit early if requested, there could be more items to process
                        return false;
                    }
                }
                _ => {
                    // Other combinations aren't possible because there will never be a Some after a None
                    unreachable!()
                }
            }
        }
    ...
    async fn process_get_fw_version(&self) -> InternalResponseData {
        ...
        let success = Self::map_slice_join(self.devices, &mut versions, |device_id, version| async move {
    ...
    async fn process_give_offer(&self, offer: &FwUpdateOffer) -> InternalResponseData {
        ...
        let success = Self::map_slice_join(
            &self.devices,
            &mut offer_responses,
            |device_id, offer_response| async move {
                let mut offer = *offer;
    ...
    async fn process_give_content(&self, content: &FwUpdateContentCommand) -> InternalResponseData {
        ...
        let success = Self::map_slice_join(
            ...
            |device_id, content_response| async move {
