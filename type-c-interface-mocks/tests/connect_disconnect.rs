//! Integration tests for [`PortMock`] `connect`/`disconnect` control helpers,
...
#[tokio::test]
async fn test_connect_sink_broadcasts_events() {
    ...
    mock.connect(PowerRole::Sink, TEST_CAPABILITY, config).unwrap();
    ...
    // Internal state should reflect the attached PSU
    assert_eq!(mock.state().psu_state, PsuState::Idle);
    assert!(mock.status().is_connected());
}

#[tokio::test]
async fn test_connect_source_broadcasts_events() {
    let type_c_channel: TypeCChannel = Channel::new();
    let power_channel: PowerChannel = Channel::new();

    let mut mock = PortMock::new("test", type_c_channel.dyn_sender(), power_channel.dyn_sender());

    mock.connect(PowerRole::Source, TEST_CAPABILITY, ConnectionConfig::default())
        .unwrap();
    ...
    assert!(data.status_event.plug_inserted_or_removed());
    assert!(data.status_event.new_power_contract_as_consumer());
    assert!(data.status_event.sink_ready());
    ...
    // Consumer capability should have been recorded for the source connection
    assert_eq!(
        mock.state().consumer_capability.map(|c| c.capability),
...
#[tokio::test]
async fn test_disconnect_broadcasts_events() {
    let type_c_channel: TypeCChannel = Channel::new();
    let power_channel: PowerChannel = Channel::new();
    ...
    mock.connect(PowerRole::Sink, TEST_CAPABILITY, ConnectionConfig::default())
        .unwrap();
...
#[tokio::test]
async fn test_disconnect_without_connection_errors() {
    let type_c_channel: TypeCChannel = Channel::new();
    let power_channel: PowerChannel = Channel::new();

    let mut mock = PortMock::new("test", type_c_channel.dyn_sender(), power_channel.dyn_sender());

    // Disconnecting a port that was never connected should error and broadcast nothing
    assert_eq!(mock.disconnect(), Err(PortMockError::NotConnected));
    assert!(power_channel.try_receive().is_err());
    assert!(type_c_channel.try_receive().is_err());
}
