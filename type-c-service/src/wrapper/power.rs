...
impl<const N: usize, C: Controller> ControllerWrapper<'_, N, C> {
    ...
    pub(super) async fn wait_power_command(
        ...
        deferred::Invocation<'_, NoopRawMutex, CommandData, InternalResponseData>,
        LocalPortId,
    ) {
        let futures: [_; N] = from_fn(|i| self.power[i].wait_invocation());
        let (invocation, local_id) = select_array(futures).await;
        trace!("Power command: device{} {:#?}", local_id, invocation.command);
        (invocation, LocalPortId(local_id as u8))
    }
