...
pub struct Service<'a> {
    ...
    power_policy_receiver: embedded_services::broadcaster::immediate::Receiver<'a, power::policy::CommsMessage>,
    /// Power policy event subscriber
    // Needs to be in a mutex to allow for independent, mutable access in the event loop
    power_policy_subscriber: Mutex<GlobalRawMutex, DynSubscriber<'a, power::policy::CommsMessage>>,
}
...
impl<'a> Service<'a> {
    ...
    pub fn create(
        ...
        Some(Self {
            ...
            power_policy_receiver: power_policy_publisher.into(),
            power_policy_subscriber: Mutex::new(power_policy_subscriber),
        })
    }
    ...
    async fn process_power_policy_event(&self, message: &PowerPolicyEvent) -> Result<(), Error> {
        match message {
            ...
            // Other events are currently a no-op
            _ => Ok(()),
        }
    }
    ...
    async fn wait_power_policy_event(&self) -> Event<'_> {
        loop {
            match self.power_policy_subscriber.lock().await.next_message().await {
    ...
    pub async fn register_comms(&'static self) -> Result<(), intrusive_list::Error> {
        ...
        power::policy::policy::register_message_receiver(&self.power_policy_receiver).await
    }
}
...
pub async fn task_closure<'a, Fut: Future<Output = ()>, F: Fn(&'a Service) -> Fut>(f: F) {
    ...
    // Guarenteed to panic since we initialized the channel above
