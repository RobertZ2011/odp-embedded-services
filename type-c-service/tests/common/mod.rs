...
pub type PowerPolicyRegistrationType<'port, 'ch> = power_policy_service::service::registration::ArrayRegistration<
    ...
    // Number of service event senders
...
macro_rules! define_port {
    ($name:ident, $mock_name:expr, $port_name:expr, $config:expr, $local_id:expr) => {
        let port0_type_c_channel: Channel<
            GlobalRawMutex,
            type_c_interface::service::event::PortEventData,
            CHANNEL_SIZE,
        > = Channel::new();
        paste! { let [<$name _type_c_sender>] = port0_type_c_channel.dyn_sender(); }
        paste! { let [<$name _type_c_receiver>] = port0_type_c_channel.dyn_receiver(); }
        ...
        paste! { let [<$name _power_policy_channel>]: Channel<
            ...
            CHANNEL_SIZE,
        > = Channel::new(); }
        paste! { let [<$name _power_policy_sender>] = paste! { [<$name _power_policy_channel>]}.dyn_sender(); }
        paste! { let [<$name _power_policy_receiver>] = paste! { [<$name _power_policy_channel>]}.dyn_receiver(); }
        ...
        paste! { let [<$name _loopback_channel>]: Channel<
            ...
            CHANNEL_SIZE,
        > = Channel::new(); }
        paste! { let [<$name _loopback_sender>] = paste! { [<$name _loopback_channel>]}.dyn_sender(); }
